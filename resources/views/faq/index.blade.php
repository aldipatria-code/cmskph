...
<section class="relative mb-8 overflow-hidden rounded-[2rem] border border-teal-100 bg-gradient-to-br from-teal-50 via-white to-cyan-50 px-6 py-10 shadow-xl shadow-teal-900/5 sm:px-10 sm:py-14">
    <div class="pointer-events-none absolute -right-16 -top-20 h-72 w-72 rounded-full bg-teal-200/40 blur-3xl"></div>
    <div class="relative max-w-4xl">
        <span class="inline-flex rounded-full bg-teal-100 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.16em] text-teal-800">{{ $siteSettings->faqHeroBadge() }}</span>
...
@if ($faqs->isNotEmpty())
    <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        @foreach ($faqs as $faq)
            <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:border-teal-200 hover:shadow-xl hover:shadow-teal-900/5">
                <div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-teal-600 text-xl font-black text-white shadow-lg shadow-teal-600/20">?</div>
